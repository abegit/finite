 ...
 */ (function ($) {
    ...
    $.extend(SocialTabsObject.prototype, {
        ...
        create: function (el, options) {
            this.defaults = {
                ...
                active: "active",
                zopen: 1E3,
                imagePath: "socialtabs/images/icons/"
            };
            this.o = {};
            this.timer_on = 0;
            ...
            } else $a.addClass("static");
            hb = this.o.height - parseInt($s.css("border-top-width"),
            10) - parseInt($s.css("padding-top"), 10) - parseInt($s.css("border-bottom-width"), 10) - parseInt($s.css("padding-bottom"), 10);
            ...
            if (this.o.method == "slide") this.dcslide($a, $t, $s, $l);
            else this.dcstatic($a, $t, $l);
        ...
        addevents: function (a, t, s, l) {
            ...
            $("a", l).click(function () {
                ...
                if ($(this).parent().hasClass(ca)) {
                    if (m == "slide") self.close(a, l, s)
                } else {
                    if (!$("li.active", t).length && m == "slide") self.open(a);
            ...
            if (this.o.method == "slide") {
                $("." + co).click(function (e) {
                    if (!a.hasClass(ca)) self.open(a);
                    var i = parseInt($(this).attr("rel"), 10) ? parseInt($(this).attr("rel"), 10) : start;
                    self.slickTabs(i, a, t, s);
                    e.preventDefault()
                });
                $("." + cc).click(function (e) {
                    if (a.hasClass(ca)) self.close(a, l,
                    s);
                    e.preventDefault()
                });
                $("." + ct).click(function (e) {
                    if (a.hasClass(ca)) self.close(a, l, s);
                    else {
                        self.open(a);
                        var i = parseInt($(this).attr("rel"), 10) ? parseInt($(this).attr("rel"), 10) : start;
                        self.slickTabs(i, a, t, s)
            ...
            $("." + cs).click(function (e) {
                if (m == "slide") if (!a.hasClass(ca)) self.open(a);
                var i = parseInt($(this).attr("rel"), 10) ? parseInt($(this).attr("rel"), 10) : start;
                self.slickTabs(i, a, t, s);
                e.preventDefault()
            });
            ...
            if (this.o.autoClose == true) $("body").mouseup(function (e) {
                if (a.hasClass(ca) && !$(e.target).parents().hasClass(cw)) if (!$(e.target).hasClass(co) || !$(e.target).hasClass(cs)) self.close(a, l, s)
            })
        },
        ...
        slickTabs: function (i, a, t, s) {
            ...
            if (!$("li:eq(" + i + ")", t).hasClass("loaded") && a.hasClass(this.o.active)) {
                ...
                if (type == "facebook" || type == "fblike" || type == "fbrec") fbLink(this.o[type + "Id"], $(".btn-type-" + type));
                else if (type == "linkedin") $.getScript("http://platform.linkedin.com/in.js?async=true", function () {
                    IN.init()
                })
            }
    ...
    function createWidget(obj, type,
    id, o, w, h) {
        var ti = obj + "-" + type,
            c = "",
            t = "",
        ...
        case "twitter":
            var cp = id.split("/");
            url = cp.length > 1 ? "https://api.twitter.com/1/lists/statuses.json?list_id=" + cp[1] + "&include_entities=true&include_rts=false&per_page=" + o.limit + "&callback=?" : "http://api.twitter.com/1/statuses/user_timeline.json?screen_name=" + cp[0] + "&include_entities=true&count=" + o.limit + "&callback=?";
        ...
        case "youtube":
            ...
            if (o.subscribe) c += '<iframe src="http://www.youtube.com/subscribe_widget?p=' + id + '" class="youtube-subscribe" scrolling="no" frameBorder="0"></iframe>';
            c += stream;
        ...
        if (type != "digg" && type != "linkedin") {
            if (o.follow) p = o.follow != "" ? '<a href="' + href + '" class="dcsmt-btn btn-type-' + type + '">' + o.follow + "</a>" : "";
            t = o.title != "" ? o.title : "";
    ...
    function getFeed(target, type, url, data, o) {
        ...
        jQuery.ajax({
            ...
            success: function (a) {
                ...
                else if (type == "flickr") a = a.items;
                else if (type == "twitter") {
                    if (a.error) error = a.error
                } else if (type == "dribbble") a = a.shots;
                else if (type == "tumblr") a = a.posts;
                ...
                if (error == "") $.each(a, function (i, item) {
                    if (i < o.limit) {
                        ...
                        case "twitter":
                            ...
                            if (o.thumb == true) html.push('<a href="http://www.twitter.com/' + item.user.screen_name + '" class="thumb"><img src="' + item.user.profile_image_url + '" alt="" /></a>');
                        ...
                        case "vimeo":
                            ...
                            if (f == "channels") {
                                if (item.logo != "") html.push('<a href="' + item.url + '" class="logo"><img src="' + item.logo + '" alt="" width="' + px + '" /></a>')
                            } else if (f == "groups") html.push('<a href="' + item.url + '" class="thumb"><img src="' + item.thumbnail + '" alt="" /></a>');
                            else {
                                var thumb = "thumbnail_" + o.thumb,
                                    at = item.title,
                                    tx = f != "albums" ? item.duration + " mins" : item.description;
                            ...
                            if (o.stats == true) {
                                ...
                                else if (f != "albums") m += '<span class="likes">' + num(item.stats_number_of_likes) + '</span><span class="views">' + num(item.stats_number_of_plays) + '</span><span class="comments">' + num(item.stats_number_of_comments) + "</span>";
                        ...
                        case "stumbleupon":
                            ...
                            if (src && o.feed == "favorites") html.push('<a href="' + item.link + '" class="thumb"><img src="' + src + '" alt="" /></a>');
                ...
                else html.push('<li class="dcsmt-error">Error. ' + error + "</li>");
                $(x).html(html.join(""))
            }
        })
    }
